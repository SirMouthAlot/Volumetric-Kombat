...
Shader "Soar/SoarShader"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            //uniform float4x4 _CameraMatrices[MAXIMUM_COLOR_CAMERA_COUNT];
     ...
     //       uniform                   bool           useModifiedDedistortion[ MAXIMUM_COLOR_CAMERA_COUNT ];
            ...
            //      uniform                   float          farCamera;
                  ...
                  float2 distort(float2 undistorted, float2 rgbSize, int intrinsicsIndex)
                  {
                      ...
                      float offset = 0;//rgbSize.y - cameraResolution.y;

                   /*   if ( useModifiedDedistortion[ intrinsicsIndex ] ) -- Azure cameras always use modfied brown conrady, other cameras use traditional (as below), but we don't currently ship them and unity makes setting those constants difficult anyway - CS' */
                      {
                          float2  fp = p * d;
                          float2  fp2 = fp * fp;
                  ...
                  float4 frag(VertexShaderOutput inputVertex, uniform float4x4 cameraViewProjection[MAXIMUM_COLOR_CAMERA_COUNT]) : SV_TARGET
                  {
                      ...
                      for (int cameraIndex = 0; cameraIndex < MAXIMUM_COLOR_CAMERA_COUNT; cameraIndex++)
                      {
                          ...
                          else
                          {
                              ...
                              float4 clipSpace = mul(float4(inputVertex.capturePosition, 1.0f), _CameraMatrices[cameraIndex]);
                              float2 projected = clipSpace.xy / clipSpace.w;

                              float confidence = 0.0f;

                              float projectionDepth = linearize_depth(cameraIndex, clipSpace.z / clipSpace.w);

                              centroid float2 colorDistorted = distort(projected, textureDimensions.xy, cameraIndex);

                              float2 dx = ddx_fine(colorDistorted * precisionFactor);
                              ...
                              for (float y = -bounds; y <= bounds; y++)
                              {
                                  for (float x = -bounds; x <= bounds; x++)
                                  {
                                      ...
                                      float depthValue = sampleDepth(cameraIndex, offsetTexel);
                      ...
                      {
                          return float4(sumColors.xyz / sumColors.w, 1.0f);
                      }
                      else
                      {
