...
namespace RuntimeHandle
{
    ...
    public class RuntimeTransformHandle : MonoBehaviour
    {
        ...
        {
            if (!_positionHandle && !_rotationHandle && !_scaleHandle)
            {
                switch (type)
                {
        ...
        void Update()
        {
            if (target != null)
            {
                ...
                if (Mouse.current.leftButton.isPressed && _draggingHandle != null)
                {
                    _draggingHandle.Interact(_previousMousePosition);
                }

                if (Mouse.current.leftButton.wasPressedThisFrame && handle != null)
                {
                    _draggingHandle = handle;
                    _draggingHandle.StartInteraction(hitPoint);
                }

                if (Mouse.current.leftButton.wasReleasedThisFrame && _draggingHandle != null)
                {
